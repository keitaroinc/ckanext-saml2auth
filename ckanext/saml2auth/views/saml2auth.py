...
def create_user(context, email, full_name):
    ...
    try:
        ...
        log.debug('CKAN user created: {}'.format(data_dict['name']))
...
def acs():
    ...
    if saml_user_email not in auth_response.ava:
        log.error(
            'Cannot find {} in the auth response. ' +
            'Make sure you have configured ckanext.saml2auth.user_email properly.'.format(
                saml_user_email
            ))
        extra_vars = {u'code': [500], u'content': 'Unable to get user email from identity provider response.'}
    ...
    if not user:
        log.debug('No such user. Creating new one.')
        ...
        log.debug('Activating deactivated user.')
    ...
    log.debug("Redirecting to: {}".format(redirect_target))
...
def saml2login():
    ...
    if len(requested_authn_contexts) > 0:
        ...
        log.debug('Prepared to authenticate with context.')
    else:
        reqid, info = client.prepare_for_authenticate(relay_state=relay_state)
        log.debug('Prepared to authenticate without context.')
